  ...
  export function createFolder(files, path, folder ) {
  ...
  export function copy(files, fromPath, toPath) {
  ...
  export function cut(files, fromPath, toPath) {
    files = copy(files, fromPath, toPath);
    files = deleteFile(files, fromPath);
    return files;
  }
  
  export function getFiles(files, path) {
    const node = findNode(files, path);
    return node ? node.children : [];
  }
  
  export function rename(files, path, newName) {
    const node = findNode(files, path);
    if (node) {
  ...
  export function addFile(files, path, newFile) {
 ...
 export function getUniqueName(files, path, name) {
    let newName = name;
    let count = 1;
